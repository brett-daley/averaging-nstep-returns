...
from agents.control.dqn import huber_loss
from agents.control.dsn import DSN
import returns.jax as returns


class DESN(DSN):
    """Deep Expected-Sarsa Network with forward-view returns"""
    ...
    def _define_update(self):
        ...
        def trajectory_loss(params, target_params, obs, actions, rewards, terminateds, truncateds, b_probs):
            ...
            action_distr = self.distr(Q_targ)
            v_targ = jnp.sum(action_distr * Q_targ, axis=1)

            t_probs = returns.vmap_select_axis1(action_distr, actions)

            g_targ, where_safe = est.calc_returns(q_targ_taken, v_targ, rewards, terminateds, truncateds, discount, t_probs, b_probs)
